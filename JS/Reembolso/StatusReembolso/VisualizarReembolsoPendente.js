document.addEventListener('DOMContentLoaded', visualizarReembolsoPendente);

function visualizarReembolsoPendente() {
    ...
    fetch('http://localhost:8080/reembolso/meusReembolsos', {
        method: 'POST',
        headers: {
        ...
        .then(reembolsos => {
            console.log('Reembolsos retornados:', reembolsos);

            reembolsos.forEach(reembolso => {
                if (reembolso.id == idReembolso){
                    document.getElementById('usuarioSolicitante').value = nome;
                    ...
                    document.getElementById('id').value = idReembolso;
                    ...
                    document.getElementById('responsavel').value = reembolso.nomeResponsavel ? reembolso.nomeResponsavel : 'Sem responsável';
                }
            });
        })
        .catch(error => {
